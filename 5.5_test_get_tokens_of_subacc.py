...
# Step 5.5. Optional. Check token balance of each generated subAccount
...
for addr in subaccounts:
    ...
    for token in tokens:
        ...
        tx = SUBACCOUNT.functions.setBalanceOfSingleToken(token.address).buildTransaction(build_tx)
        signed_tx = w3.eth.account.sign_transaction(tx, private_key = TOKENS_OWNER_PK)
        tx_hash = w3.eth.send_raw_transaction(signed_tx.rawTransaction)
        tx_receipt = w3.eth.wait_for_transaction_receipt(tx_hash)

        balance = SUBACCOUNT.functions.getBalanceOfSingleToken(token.address).call()
        print(f'    Token: {token.functions.name().call()}; Amount: {balance}')
